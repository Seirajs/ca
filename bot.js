...
client.on("guildDelete", guild => {
  ...
  client.channels.get("715638491563950110").send(rrrsembed);
});
...
client.on("guildCreate", guild => {
  const embed = new Discord.RichEmbed()
    ...
    .setTimestamp();
  client.channels.get("715638491563950110").send(embed);
});
...
let bal  = db.fetch(`davet_${invite.inviter.id + member.guild.id}`)
   member.guild.channels.get(channel).send(`:inbox_tray: ** <@${member.id}> Sunucuya Katıldı**; Tarafından **${davetçi.tag}** (`+'**'+bal+'** Daveti Var!)')
  })
  ...
  if (cfxz == 'Açık') {
      ...
      if (cfxk === null) {
        let cfxu = new Discord.RichEmbed()
                  .setTitle(`**Bhaoutuan Bot Kanal Koruma Sistemi**`)
        .setColor("#00ff88")
        .setFooter(`Bhaoutuan Bot`)
    ...
    if (logs.entries.first().executor.id === "667302067588825088") return;
          if (logs.entries.first().executor.id === "557959615145312257") return;
    ...
    const cfxj = new Discord.RichEmbed()
          .setTitle(`**Bhaoutuan Kanal Koruma Sistemi**`)
          .setColor("#00ff88")
...
client.on("channelDelete", async function(channel) {
if(channel.guild.id !== "683401421634535432") return;
    let logs = await channel.guild.fetchAuditLogs({type: 'CHANNEL_DELETE'});
    if(logs.entries.first().executor.bot) return;
    channel.guild.member(logs.entries.first().executor).roles.filter(role => role.name !== "@everyone").array().forEach(role => {
              channel.guild.member(logs.entries.first().executor).removeRole(channel.guild.roles.get("709756437366177835"))
              channel.guild.member(logs.entries.first().executor).removeRole(channel.guild.roles.get("710781683900612658"))
    })
const sChannel = channel.guild.channels.find(c=> c.id ==="713382569374253076")
...
.setFooter('Developer By Bhaoutuan')
sChannel.send(cıks)
...
////////////////sumucu ismi sayaç////////////////////////////
client.on('guildMemberAdd', async member => {
let A = await db.fetch(`℣odeming #Yeniden ${member.guild.id}`);
if (A === "kapali") return;
if (A === "acik") {
member.guild.setName(`℣odeming #Yeniden ${member.guild.memberCount}`)
}; 
});
...
client.on("guildMemberAdd", async member => {
   const fs = require('fs');
    let gkanal = JSON.parse(fs.readFileSync("./ayarlar/glogsas.json", "utf8"));
    const gözelkanal = member.guild.channels.get(gkanal[member.guild.id].resim)
    if (!gözelkanal) return;
     let username = member.user.username;
        if (gözelkanal === undefined || gözelkanal === null) return;
...
client.on("userUpdate", async(oldCAD, newCAD) => {
  ...
  let cadNORMAL = "723890468991008789"; // Normal PP'lerin Atılacağı Kanal ID'si
  let cadGIF = "723890468991008789"; // Gif PP'lerin Atılacağı Kanal ID'si
...
client.on("ready", () => {
  client.channels.get("726058155766841384").join();
   //main dosyaya atılacak
})

client.on('guildMemberAdd', async(member) => {
const kanal = `Son Uye • ${member.user.username}`
let channel = client.channels.get("723902601225044090") //KANAL İD
